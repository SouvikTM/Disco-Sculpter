...
const MoldingSphere: React.FC<MoldingSphereProps> = ({ config, resetTrigger }) => {
  ...
  // Initialize Data
  // physicsPositions: The actual simulation state (where particles are physically)
  // velocities: The physics velocity
  // originalPositions: Reference for Liquid mode spring back
  ...
  }, [config.colorA, config.colorB, config.particleCount, config.effect, originalPositions]);
  ...
  useFrame((state, delta) => {
    ...
    // 1. Raycasting
    // Note: We raycast against the invisible sphere. 
    // Since we rotate the GROUP now, we need to respect the world transform.
    ...
    if (isLeftMouseDown) {
      ...
      if (intersects.length > 0) {
        // Convert world point to local point inside the group
    ...
    // Physics Settings
    // Solid: High friction (stops moving when force stops), No spring.
    // Liquid: Friction depends on viscosity, Spring strength depends on tension.
    const drag = isLiquid 
      ? 1.0 - (config.viscosity * 3.5) // Liquid drag
      : 0.90; // Solid "clay" drag (very high friction)
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Stop micro-movements in solid mode to prevent creep
      ...
      // --- D. Visual Effects (Non-Destructive) ---
      // We calculate render position by adding effects to physics position.
      // We do NOT save this back to physicsPositions.

      let rx = px;
      let ry = py;
      let rz = pz;

      // Base idle noise (small breathing)
      const noiseAmp = 0.02;
    ...
    // Rotate the entire group slowly for view, instead of the mesh/particles individually
    if (groupRef.current) {
  ...
  return (
    <group ref={groupRef}>
      <points ref={pointsRef}>
        <bufferGeometry>
          <bufferAttribute
            ...
            array={new Float32Array(physicsPositions.length)} // Initial buffer, updated by useFrame
            itemSize={3}
          />
          <bufferAttribute 
      ...
      {/* Invisible Mesh for Raycasting - Must match initial shape */}
